# Software Maintenance Agent Build Plan

Software Maintenance Agent turns failing CI and bounded maintenance issues into small, tested patch candidates with execution traces, risk scoring, rollback notes, and benchmarked retrieval.
...
- The runner continues nonblocked work when optional services such as E2B, GitHub PR creation, or external CI are unavailable.
...
| G10 Documentation | README, demo command, successful report, benchmark output | `python -m pytest`; demo commands below |

## Fallback Paths

- Missing `GEMINI_API_KEY`: use deterministic local patch planning for trusted fixtures and record the provider status.
