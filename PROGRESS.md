...
- `python -m patchpilot.cli run --task examples/tasks/python_email_empty.json --sandbox local` => pass
- `python -m patchpilot.cli run --task examples/tasks/python_email_repair.json --sandbox local` => pass
- `python -m patchpilot.cli benchmark --suite benchmark/suites/mvp.json` => pass
- `python -m patchpilot.cli trace --run-id 20260511_213624_fixture_email_empty` => pass
- `python -m patchpilot.cli run --task examples/tasks/python_email_empty.json --sandbox e2b` => pass; blocked report generated because E2B is not configured
- `python -m ruff check .` => pass
- `python -m patchpilot.cli dashboard --port 8765` => pass; dashboard server started on `http://127.0.0.1:8765`
...
- The Windows user scripts directory is not on PATH, so `ama` may not resolve in this shell. `python -m patchpilot.cli ...` works reliably.
